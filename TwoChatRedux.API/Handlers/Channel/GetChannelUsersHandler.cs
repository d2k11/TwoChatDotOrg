...
public class GetChannelUsersHandler : AstroHandler
{
    public override async Task<object> Handle(HttpRequest request)
    {
        ...
        ChatChannel? channel = ChatChannelManager.Find(query);
        if (channel is null)
        {
            return Fail("Channel not found");
        }

        return ChatChannelManager.GetUsersInChannel(channel.id);
    }
}